...
function ParticipantesTab({obra,partic,toast,reload}){
  ...
  const save=async()=>{if(!draft.email.trim()||!draft.nombre.trim())return;setSaving(true);const{data:{session}}=await supabase.auth.getSession();if(!session){toast.error("Sesión inválida, volvé a loguearte");setSaving(false);return;}const{data,error}=await supabase.functions.invoke("invite-user",{body:{email:draft.email.trim(),nombre:draft.nombre.trim(),rol:draft.rol,puede_cargar:draft.rol!=="cliente"?true:draft.puede_cargar,obra_id:obra.id,obra_nombre:obra.nombre}});if(error||data?.error){toast.error("Error: "+(data?.error||error.message));setSaving(false);return;}toast.success(`Invitación enviada a ${draft.email}`);setDraft({email:"",nombre:"",rol:"cliente",puede_cargar:false});setModal(false);await reload();setSaving(false);};
