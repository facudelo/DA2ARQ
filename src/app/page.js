...
// ── DESIGN TOKENS ─────────────────────────────────────────────────────────────
const C = {
  bg:"#F4F6F1", bg2:"#FFFFFF", bg3:"#EBF0E4", bg4:"#DCE8D0",
...
html,body{background:${C.bg};color:${C.t};font-family:'Helvetica Neue',Arial,sans-serif;font-size:13px}
::-webkit-scrollbar{width:4px;height:4px}
::-webkit-scrollbar-thumb{background:rgba(42,80,28,0.18);border-radius:4px}
select option{background:#fff;color:${C.t}}
...
const fmtUSD   = n => new Intl.NumberFormat("es-AR",{style:"currency",currency:"USD",maximumFractionDigits:0}).format(n);
...
function useToast() {
  ...
  const add = useCallback((msg,type,dur=3500) => {
...
const TOAST_S = {
  success:{bg:"#1a3d0a",border:"#2E6E18",icon:"✓",col:"#7CBF3A"},
  error:  {bg:"#3d0a0a",border:"#B84A3A",icon:"✕",col:"#e06c5a"},
  info:   {bg:"#0a2a3d",border:"#2A6A5A",icon:"ℹ",col:"#4a9a8a"},
};

function ToastContainer({toasts}) {
  return <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,display:"flex",flexDirection:"column",gap:10,pointerEvents:"none"}}>
    {toasts.map(t=>{
      const s=TOAST_S[t.type]||TOAST_S.info;
      return <div key={t.id} style={{background:s.bg,border:`1px solid ${s.border}`,borderLeft:`3px solid ${s.col}`,borderRadius:10,padding:"11px 16px",display:"flex",alignItems:"center",gap:10,minWidth:260,maxWidth:380,boxShadow:"0 4px 20px rgba(0,0,0,.35)"}}>
        <span style={{width:22,height:22,borderRadius:"50%",background:s.col+"22",border:`1px solid ${s.col}55`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:s.col,fontWeight:700,flexShrink:0}}>{s.icon}</span>
        <span style={{fontSize:13,color:"#e8f0e0",lineHeight:1.4}}>{t.msg}</span>
      </div>;
    })}
  </div>;
}
...
function Logo({size=32}) {
  const s=size,bH=s*.09,gap=s*.045,n=6,lW=s*.44,rW=s*.36,sx=s*.06,rx=s*.54;
  const th=n*bH+(n-1)*gap,sy=(s-th)/2;
  return <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} fill="none" style={{flexShrink:0}}>
    {Array.from({length:n},(_,i)=>{
      const y=sy+i*(bH+gap),lw=lW*(.6+.4*((n-1-i)/(n-1))),rw=rW*(.4+.6*(i/(n-1)));
      return <g key={i}><rect x={sx} y={y} width={lw} height={bH} fill={C.t} rx={1}/><rect x={rx} y={y} width={rw} height={bH} fill={C.lima} rx={1}/></g>;
    })}
  </svg>;
}
...
function Btn({children,onClick,primary,small,danger,disabled,full,loading}) {
  return <button onClick={onClick} disabled={disabled||loading} style={{
    background:primary?C.green:danger?C.red+"18":C.bg3,
    color:primary?"#fff":danger?C.red:C.t2,
    border:`1px solid ${primary?"transparent":danger?C.red+"55":C.bd2}`,
    borderRadius:7,padding:small?"5px 12px":"8px 18px",
    cursor:(disabled||loading)?"not-allowed":"pointer",
    fontSize:small?11:13,fontWeight:primary?600:500,
    opacity:(disabled||loading)?.6:1,
    width:full?"100%":"auto",whiteSpace:"nowrap",
    display:"inline-flex",alignItems:"center",gap:6,justifyContent:"center",
  }}>
    {loading&&<span className="spin" style={{width:12,height:12,border:"2px solid currentColor",borderTopColor:"transparent",borderRadius:"50%",display:"inline-block"}}/>}
    {children}
  </button>;
}

function Tag({label,color}) {
  return <span style={{background:color+"20",color,fontSize:10,padding:"2px 9px",borderRadius:20,fontWeight:600,whiteSpace:"nowrap",border:`1px solid ${color}33`}}>{label}</span>;
}

function Card({children,style={}}) {
  return <div style={{background:C.bg2,border:`1px solid ${C.bd}`,borderRadius:14,padding:"18px 20px",boxShadow:"0 1px 6px rgba(42,80,28,.07)",...style}}>{children}</div>;
}

function StatCard({label,value,sub,color,icon}) {
  return <div style={{background:C.bg2,borderTop:`3px solid ${color}`,border:`1px solid ${C.bd}`,borderRadius:12,padding:"14px 18px",flex:"1 1 140px",minWidth:130}}>
    <div style={{fontSize:10,color:C.t3,textTransform:"uppercase",letterSpacing:".08em",marginBottom:7,display:"flex",alignItems:"center",gap:5}}><span>{icon}</span>{label}</div>
...
function Spinner() {
  return <div style={{display:"flex",justifyContent:"center",padding:"48px 0"}}>
    <div className="spin" style={{width:32,height:32,border:`3px solid ${C.bd2}`,borderTopColor:C.green,borderRadius:"50%"}}/>
  </div>;
}
...
function Modal({title,onClose,children,wide}) {
  ...
  return <div onClick={e=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"rgba(10,30,5,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
    <div style={{background:C.bg2,border:`1px solid ${C.bd2}`,borderRadius:16,padding:24,width:"100%",maxWidth:wide?720:480,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,.18)"}}>
...
function Donut({data,size=120}) {
  ...
  if(!total) return <div style={{width:size,height:size,borderRadius:"50%",background:C.bg3,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:C.t3}}>Sin datos</div>;
  let ang=-Math.PI/2;
  const r=size/2,ir=r*.58,cx=r,cy=r;
  return <svg width={size} height={size}>
    {data.map((d,i)=>{
      ...
      return <path key={i} d={`M${x1},${y1} A${r},${r},0,${a>Math.PI?1:0},1,${x2},${y2} L${ix2},${iy2} A${ir},${ir},0,${a>Math.PI?1:0},0,${ix1},${iy1} Z`} fill={d.color} opacity={.88}/>;
    })}
    <circle cx={cx} cy={cy} r={ir-2} fill={C.bg2}/>
  </svg>;
}
...
export default function App() {
  ...
  const fetchIPC = async()=>{
    if(inflData)return;
    try{const r=await fetch("https://api.argentinadatos.com/v1/finanzas/indices/inflacion");const j=await r.json();if(Array.isArray(j))setInflData(j);}catch{}
  };

  const fetchTCHist = async()=>{
    ...
    try{
      ...
      if(Array.isArray(jOf)&&Array.isArray(jBl))setTcHistData({oficial:jOf,blue:jBl});
    }catch{}
  };
  ...
  return <>
    <style>{gCSS}</style>
    <ObraApp user={user} obra={obraActiva} tab={tab} setTab={setTab}
      tcOficial={tcOficial} tcBlue={tcBlue} tcManual={tcManual} setTcManual={setTcManual}
      tcLoading={tcLoading} fetchTCs={fetchTCs}
      inflData={inflData} fetchIPC={fetchIPC}
      tcHistData={tcHistData} fetchTCHist={fetchTCHist}
      toast={toast}
      onBack={()=>setObraActiva(null)}
      onLogout={async()=>{await supabase.auth.signOut();setUser(null);setObraActiva(null);}}
    />
...
function AuthScreen({onLogin,toast}) {
  ...
  const handleLogin = async()=>{
    ...
    toast.success("Bienvenido de nuevo");onLogin(data.user);setLoading(false);
  };

  const handleRegister = async()=>{
    if(!email||!pass||!nombre)return setErr("Completá todos los campos.");
  ...
  return <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{width:"100%",maxWidth:380}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        <div style={{display:"flex",justifyContent:"center",marginBottom:14}}><Logo size={80}/></div>
      ...
      <Card>
        <div style={{display:"flex",gap:8,marginBottom:20,background:C.bg3,borderRadius:8,padding:4}}>
          {["login","register"].map(m=><button key={m} onClick={()=>{setMode(m);setErr("");}} style={{flex:1,padding:"7px",fontSize:12,border:"none",borderRadius:6,cursor:"pointer",background:mode===m?C.bg2:"transparent",color:mode===m?C.t:C.t3,fontWeight:mode===m?700:400,transition:"all .2s"}}>
            {m==="login"?"Ingresar":"Crear cuenta"}
          </button>)}
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {mode==="register"&&<div>
            <div style={{fontSize:11,color:C.t2,marginBottom:5,fontWeight:600}}>Nombre completo</div>
            <input style={INP} placeholder="Ej: Juan García" value={nombre} onChange={e=>setNombre(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()}/>
          </div>}
          <div>
            <div style={{fontSize:11,color:C.t2,marginBottom:5,fontWeight:600}}>Email</div>
            <input style={INP} type="email" placeholder="tu@email.com" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()}/>
          </div>
          <div>
            <div style={{fontSize:11,color:C.t2,marginBottom:5,fontWeight:600}}>Contraseña</div>
...
function ObrasScreen({user,onSelect,onLogout,toast}) {
  ...
  const loadObras = useCallback(async()=>{
    ...
    const{data,error}=await supabase
      .from("obras")
      .select("*, participantes!inner(rol,puede_cargar,user_id)")
      .eq("participantes.user_id",user.id)
  ...
  const save = async()=>{
    ...
    const{data:obra,error:obraErr}=await supabase.from("obras").insert({
      nombre:draft.nombre.trim(),direccion:draft.direccion,estado:draft.estado,
      presupuesto_total:parseFloat(draft.presupuesto_total)||0,
      moneda_presupuesto:draft.moneda_presupuesto,created_by:user.id,
    }).select().single();
    if(obraErr){toast.error("Error al crear la obra: "+obraErr.message);setSaving(false);return;}
    const profile=await supabase.from("profiles").select("nombre").eq("id",user.id).single();
    await supabase.from("participantes").insert({
      obra_id:obra.id,user_id:user.id,email:user.email,
      nombre:profile.data?.nombre||user.email,rol:"arquitecto",puede_cargar:true,
    });
    await supabase.rpc("crear_categorias_default",{p_obra_id:obra.id});
    toast.success("Obra creada correctamente");
    setDraft({nombre:"",direccion:"",estado:"En ejecución",presupuesto_total:"",moneda_presupuesto:"ARS"});
  ...
  return <div style={{minHeight:"100vh",background:C.bg}}>
    <div style={{background:C.bg2,borderBottom:`1px solid ${C.bd}`,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:54}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}><Logo size={36}/><div><div style={{fontWeight:800,fontSize:15,color:C.t}}>DA2ARQ</div><div style={{fontSize:10,color:C.t3}}>Gestión de obra</div></div></div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <span className="hide-mobile" style={{fontSize:12,color:C.t2}}>{user.email}</span>
    ...
    <div style={{padding:24,maxWidth:960,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {obras.map(o=>{
          const rol=miRol(o);
          return <div key={o.id} className="fu" onClick={()=>onSelect(o)}
            style={{background:C.bg2,border:`1px solid ${C.bd2}`,borderLeft:`4px solid ${C.green}`,borderRadius:12,padding:"18px 20px",cursor:"pointer",transition:"box-shadow .2s"}}
            onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 20px rgba(42,110,24,.13)"}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <Tag label={o.estado} color={C.green}/>
            </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <span style={{fontSize:11,color:C.t3}}>{o.created_at?.slice(0,10)}</span>
            </div>
          </div>;
        })}
      </div>
    ...
    {modal&&<Modal title="Nueva Obra" onClose={()=>setModal(false)}>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div><div style={{fontSize:11,color:C.t2,marginBottom:4,fontWeight:600}}>Estado</div>
          <select style={SEL} value={draft.estado} onChange={e=>setDraft(d=>({...d,estado:e.target.value}))}>
            {["Relevamiento","En proyecto","En ejecución","Finalizada"].map(s=><option key={s}>{s}</option>)}
          </select>
        </div>
...
function ObraApp(props) {
  ...
  return <div style={{minHeight:"100vh",background:C.bg}}>
    ...
    <div style={{padding:20,paddingBottom:48,maxWidth:1040,margin:"0 auto"}}>
      {loadingData?<Spinner/>:<>
        {tab==="dashboard"    &&<DashboardTab obra={obra} gastos={gastosVis} esAdmin={esAdmin} presup={presup} tcRef={tcRef} partic={partic} cats={cats} fotos={fotos}/>}
        {tab==="gastos"       &&<GastosTab user={user} obra={obra} gastos={gastos} esAdmin={esAdmin} puedoCargar={puedoCargar} tcOficial={tcOficial} tcBlue={tcBlue} tcManual={tcManual} setTcManual={setTcManual} cats={cats} toast={toast} reload={loadAll}/>}
        {tab==="presupuesto"  &&esAdmin&&<PresupuestoTab obra={obra} gastos={gastos} presup={presup} tcRef={tcRef} cats={cats} toast={toast} reload={loadAll}/>}
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function DashboardTab({obra,gastos,esAdmin,presup,tcRef,partic,cats,fotos}) {
  const totalARS    = gastos.reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);
  const presupTotal = obra.presupuesto_total?(obra.moneda_presupuesto==="USD"?obra.presupuesto_total*tcRef:obra.presupuesto_total):0;
  const pct         = presupTotal>0?Math.min(Math.round((totalARS/presupTotal)*100),200):null;
  const byCat       = useMemo(()=>{const r={};cats.forEach(c=>{r[c.id]=gastos.filter(g=>g.cat_id===c.id).reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);});return r;},[gastos,cats,tcRef]);
  ...
  return <div className="fu">
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
      <StatCard label="Ejecutado total" value={fmtARS(totalARS)} sub={`${gastos.length} movimientos`} color={C.green} icon="💸"/>
      {presupTotal>0&&<StatCard label="Presupuesto" value={fmtM(obra.presupuesto_total,obra.moneda_presupuesto)} sub={pct!==null?`${pct}% ejecutado`:""} color={C.blue} icon="📐"/>}
      {presupTotal>0&&<StatCard label="Disponible" value={fmtARS(Math.max(0,presupTotal-totalARS))} sub={pct>=90?"⚠ Límite cercano":""} color={presupTotal-totalARS<0?C.red:C.lima} icon="📊"/>}
      <StatCard label="Participantes" value={partic.length} sub={`${fotos.length} foto${fotos.length!==1?"s":""}`} color={C.amber} icon="👥"/>
    </div>
    {presupTotal>0&&<Card style={{marginBottom:14}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.t3}}>
        <span>Ejecutado: <b style={{color:C.t}}>{fmtARS(totalARS)}</b></span>
        <span style={{color:pct>=100?C.red:pct>=80?C.amber:C.green,fontWeight:700}}>{pct}%</span>
        <span>Presupuesto: <b style={{color:C.t}}>{fmtM(obra.presupuesto_total,obra.moneda_presupuesto)}</b></span>
      </div>
    </Card>}
    <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:120}}>
            {[...donutData].sort((a,b)=>b.val-a.val).map(d=>{
              const p2=totalARS>0?Math.round((d.val/totalARS)*100):0;
              return <div key={d.label} style={{display:"flex",justifyContent:"space-between",marginBottom:7,fontSize:12}}>
                <span style={{display:"flex",alignItems:"center",gap:5}}><span style={{width:9,height:9,borderRadius:2,background:d.color,display:"inline-block"}}/><span style={{color:C.t2}}>{d.label}</span></span>
                <span style={{color:d.color,fontWeight:700}}>{p2}%</span>
              </div>;
            })}
          </div>
        </div>}
      ...
      <Card style={{flex:"2 1 280px"}}>
        <div style={{fontSize:11,color:C.t3,textTransform:"uppercase",letterSpacing:".07em",marginBottom:12,fontWeight:600}}>Gasto por categoría</div>
        {cats.filter(c=>byCat[c.id]>0).sort((a,b)=>(byCat[b.id]||0)-(byCat[a.id]||0)).map(c=>{
          const p2=totalARS>0?Math.round((byCat[c.id]/totalARS)*100):0;
          return <div key={c.id} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
              ...
              <span style={{color:c.color,fontWeight:700}}>{fmtARS(byCat[c.id])}</span>
            </div>
        ...
        {ultimos.map(g=>{
          ...
          return <div key={g.id} style={{display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${C.bd}`,paddingBottom:9,marginBottom:9}}>
            ...
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:13,fontWeight:700,color:cat?.color||C.green}}>{fmtM(g.monto,g.moneda)}</div>
...
// ── GASTOS ────────────────────────────────────────────────────────────────────
function GastosTab({user,obra,gastos,esAdmin,puedoCargar,tcOficial,tcBlue,tcManual,setTcManual,cats,toast,reload}) {
  ...
  const total=filtered.reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);
  ...
  const save=async()=>{
    ...
    const{error}=await supabase.from("gastos").insert({
      ...
      descripcion:draft.descripcionripcion,monto:parseFloat(draft.monto),moneda:draft.moneda,
    ...
    else{toast.success("Gasto registrado");await reload();}
    setDraft(initD());setShowForm(false);setSaving(false);
  };

  const saveEdit=async()=>{
    if(!editM||parseFloat(editM.monto)<=0)return;
    setSaving(true);
    const{error}=await supabase.from("gastos").update({
      fecha:editM.fecha,cat_id:editM.cat_id,sub_id:editM.sub_id,
      descripcion:editM.descripcionripcion,monto:parseFloat(editM.monto),moneda:editM.moneda,visibilidad:editM.visibilidad,
    }).eq("id",editM.id);
  ...
  const deleteG=async(id)=>{
    ...
    else{toast.success("Gasto eliminado");await reload();}
  };

  const doExport=()=>exportCSV(filtered.map(g=>({
    Fecha:g.fecha,Categoria:cats.find(c=>c.id===g.cat_id)?.label||"",
    Subcategoria:cats.find(c=>c.id===g.cat_id)?.subs?.find(s=>s.id===g.sub_id)?.label||"",
    Descripcion:g.descripcionripcion||"",Monto:g.monto,Moneda:g.moneda,TC_Tipo:g.tc_tipo,TC_Valor:g.tc_valor,
    Monto_ARS:g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto,
    Visibilidad:g.visibilidad,Cargado_por:g.cargado_por,
  })),`gastos_${obra.nombre.replace(/\s+/g,"_")}.csv`);

  return <div className="fu">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
      <div><div style={{fontSize:16,fontWeight:700,color:C.t}}>Gastos</div><div style={{fontSize:12,color:C.t3}}>{filtered.length} registros · {fmtARS(total)}</div></div>
    ...
    {showForm&&<Card style={{marginBottom:14}}>
      ...
      <div style={{background:C.bg3,border:`1px solid ${C.bd}`,borderRadius:10,padding:"10px 14px",marginBottom:14}}>
        <div style={{fontSize:11,color:C.t2,marginBottom:8,fontWeight:600}}>Tipo de cambio:</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {tcTipo==="manual"&&<input type="number" value={tcManual} onChange={e=>setTcManual(+e.target.value)} style={{...INP,width:90,padding:"4px 8px",fontSize:12}} placeholder="TC manual"/>}
        </div>
      </div>
    ...
    <Card>
      ...
      <div className="hide-mobile" style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:580}}>
          <thead><tr style={{borderBottom:`2px solid ${C.bd2}`}}>
            {["Fecha","Categoría","Subcategoría","Descripción","Monto","TC",esAdmin?"Vis.":null,"Por",""].filter(Boolean).map((h,i)=>(
          ...
          <tbody>
            {filtered.map(g=>{
              ...
              return <tr key={g.id} style={{borderBottom:`1px solid ${C.bd}`}}>
                ...
                <td style={{padding:"9px 10px",fontWeight:700,color:cat?.color||C.green,whiteSpace:"nowrap"}}>{fmtM(g.monto,g.moneda)}</td>
      ...
      <div className="show-mobile" style={{flexDirection:"column",gap:0}}>
        {filtered.map(g=>{
          const cat=cats.find(c=>c.id===g.cat_id);
          const sub=cat?.subs?.find(s=>s.id===g.sub_id);
          return <div key={g.id} style={{borderBottom:`1px solid ${C.bd}`,padding:"12px 0",display:"flex",gap:10,alignItems:"flex-start"}}>
            <div style={{width:36,height:36,borderRadius:9,background:(cat?.color||C.green)+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{cat?.icon||"📦"}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:700,color:cat?.color||C.green}}>{fmtM(g.monto,g.moneda)}</div>
              <div style={{fontSize:12,color:C.t2,marginTop:1}}>{sub?.label||cat?.label}</div>
              {g.descripcion&&<div style={{fontSize:11,color:C.t3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.descripcion}</div>}
              <div style={{fontSize:10,color:C.t3,marginTop:2}}>{g.fecha} · TC {g.tc_tipo}</div>
            </div>
            {esAdmin&&<button onClick={()=>deleteG(g.id)} style={{background:"none",border:"none",cursor:"pointer",color:C.t3,fontSize:20,padding:"0 4px"}}>×</button>}
          </div>;
        })}
        {filtered.length===0&&<div style={{textAlign:"center",padding:"28px 0",color:C.t3}}>Sin resultados</div>}
      </div>
...
function FotosTab({obra,fotos,puedoCargar,user,toast,reload}) {
  ...
  const handleFile=e=>{
    ...
    setDraft(d=>({...d,file,preview:URL.createObjectURL(file),nombre:file.name}));
  };
  ...
  const save=async()=>{
    ...
    const{error}=await supabase.from("fotos").insert({
      obra_id:obra.id,titulo:draft.titulo.trim(),fecha:draft.fecha,etapa:draft.etapa,
    ...
    setDraft({titulo:"",fecha:todayISO(),etapa:"",file:null,preview:null,nombre:""});
  ...
  const deleteFoto=async(foto)=>{
    ...
    if(error)toast.error("Error al eliminar");
    else{toast.success("Foto eliminada");await reload();}
  };
  ...
  return <div className="fu">
    ...
    {etapas.length>0&&<div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
      {["todas",...etapas].map(e=><button key={e} onClick={()=>setEtapaF(e)} style={{padding:"5px 14px",fontSize:12,border:`1px solid ${etapaF===e?C.green:C.bd2}`,borderRadius:20,cursor:"pointer",background:etapaF===e?C.green+"18":"transparent",color:etapaF===e?C.green:C.t2,fontWeight:etapaF===e?700:400}}>{e==="todas"?"Todas":e}</button>)}
    </div>}
    {filtradas.length===0&&<Card><div style={{textAlign:"center",padding:"48px 0"}}>
      <div style={{fontSize:44,marginBottom:12}}>📷</div>
      <div style={{fontSize:14,fontWeight:600,color:C.t2,marginBottom:6}}>Sin fotos cargadas</div>
      <div style={{fontSize:12,color:C.t3,marginBottom:16}}>Subí fotos para registrar el avance de la obra</div>
      {puedoCargar&&<Btn primary onClick={()=>setModal(true)}>+ Subir primera foto</Btn>}
    </div></Card>}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:14}}>
      {filtradas.map(f=>(
        <div key={f.id} style={{background:C.bg2,border:`1px solid ${C.bd}`,borderRadius:14,overflow:"hidden",boxShadow:"0 1px 6px rgba(42,80,28,.07)"}}>
          <div style={{position:"relative"}}>
            ...
            {puedoCargar&&<button onClick={()=>deleteFoto(f)} style={{position:"absolute",top:8,right:8,background:"rgba(0,0,0,.55)",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",color:"#fff",fontSize:12}}>×</button>}
          </div>
          <div style={{padding:"12px 14px"}}>
            <div style={{fontWeight:700,fontSize:13,color:C.t,marginBottom:2}}>{f.titulo}</div>
            <div style={{fontSize:11,color:C.t3}}>{f.fecha}</div>
          </div>
        </div>
      ))}
    </div>
    {modal&&<Modal title="Subir foto" onClose={()=>setModal(false)}>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div onClick={()=>fileRef.current.click()} style={{border:`2px dashed ${draft.preview?C.green:C.bd2}`,borderRadius:12,padding:"24px 16px",textAlign:"center",cursor:"pointer",background:draft.preview?C.bg3:"transparent"}}>
          {draft.preview?<>
            <img src={draft.preview} alt="" style={{maxHeight:150,maxWidth:"100%",borderRadius:8,objectFit:"contain",marginBottom:8}}/>
            <div style={{fontSize:12,color:C.green,fontWeight:600}}>✓ {draft.nombre}</div>
          </>:<>
            <div style={{fontSize:40,marginBottom:8}}>📷</div>
            <div style={{fontSize:13,fontWeight:600,color:C.t2}}>Seleccionar foto</div>
            <div style={{fontSize:11,color:C.t3}}>JPG, PNG, WEBP</div>
          </>}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div><div style={{fontSize:11,color:C.t2,marginBottom:4,fontWeight:600}}>Etapa</div>
            ...
            <datalist id="etapas-dl">{["Demolición","Excavación","Estructura","Mampostería","Instalaciones","Carpintería","Terminaciones","Exterior","Final"].map(e=><option key={e} value={e}/>)}</datalist>
          </div>
        </div>
...
function CategoriasTab({cats,obra,toast,reload}) {
  ...
  const [catD,setCatD]=useState({label:"",color:C.green,icon:"📦"});
  const [subD,setSubD]=useState({label:""});
  const [saving,setSaving]=useState(false);
  const ICONS=["🏗️","👷","📦","🔧","⚙️","🪵","🏠","💡","🛁","🪟","🚪","🔌","🪣","🏛️","📐","📋","🚛","🔨","🧱","🪚"];
  const COLS=[C.green,C.lima,C.blue,C.amber,C.red,"#7A4A9A","#2A8A7A","#9A6A2A"];

  const saveCat=async()=>{
    if(!catD.label.trim())return;setSaving(true);
    if(catM==="new"){
      const{error}=await supabase.from("categorias").insert({obra_id:obra.id,label:catD.label.trim(),color:catD.color,icon:catD.icon,orden:cats.length+1});
      if(error){toast.error("Error al crear");setSaving(false);return;}
      toast.success("Categoría creada");
    }else{
      const{error}=await supabase.from("categorias").update({label:catD.label,color:catD.color,icon:catD.icon}).eq("id",catM);
      if(error){toast.error("Error al actualizar");setSaving(false);return;}
      toast.success("Categoría actualizada");
    }
    await reload();setCatM(null);setSaving(false);
  };

  const saveSub=async()=>{
    if(!subD.label.trim())return;setSaving(true);
    if(subM.mode==="new"){
      const{error}=await supabase.from("subcategorias").insert({cat_id:subM.catId,label:subD.label.trim(),orden:0});
      if(error){toast.error("Error");setSaving(false);return;}
      toast.success("Subcategoría creada");
    }else{
      const{error}=await supabase.from("subcategorias").update({label:subD.label}).eq("id",subM.subId);
      if(error){toast.error("Error");setSaving(false);return;}
      toast.success("Actualizada");
    }
    await reload();setSubM(null);setSaving(false);
  };
  ...
  return <div className="fu">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div><div style={{fontSize:16,fontWeight:700,color:C.t}}>Categorías</div><div style={{fontSize:12,color:C.t3}}>Clasificación de gastos</div></div>
      <Btn primary onClick={()=>{setCatD({label:"",color:C.green,icon:"📦"});setCatM("new");}}>+ Nueva categoría</Btn>
    </div>
    {cats.map(cat=>(
      <Card key={cat.id} style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${C.bd}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:38,height:38,borderRadius:10,background:cat.color+"18",border:`1px solid ${cat.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{cat.icon}</div>
            <div><div style={{fontWeight:700,fontSize:14,color:C.t}}>{cat.label}</div><div style={{fontSize:11,color:C.t3}}>{cat.subs?.length||0} subcategorías</div></div>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <Btn small onClick={()=>{setSubD({label:""});setSubM({catId:cat.id,mode:"new"});}}>+ Sub</Btn>
            <Btn small onClick={()=>{setCatD({label:cat.label,color:cat.color,icon:cat.icon});setCatM(cat.id);}}>Editar</Btn>
            <Btn small danger onClick={()=>deleteCat(cat.id)}>Eliminar</Btn>
          </div>
        </div>
        <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
          {(cat.subs||[]).map(sub=>(
            <div key={sub.id} style={{display:"flex",alignItems:"center",gap:6,background:C.bg3,border:`1px solid ${cat.color}33`,borderRadius:20,padding:"4px 10px 4px 12px"}}>
              <span style={{fontSize:12,color:C.t2,fontWeight:500}}>{sub.label}</span>
              <button onClick={()=>{setSubD({label:sub.label});setSubM({catId:cat.id,subId:sub.id,mode:"edit"});}} style={{background:"none",border:"none",cursor:"pointer",color:C.t3,fontSize:11}}>✎</button>
              <button onClick={()=>deleteSub(sub.id)} style={{background:"none",border:"none",cursor:"pointer",color:C.t3,fontSize:14}}>×</button>
            </div>
          ))}
          {(!cat.subs||cat.subs.length===0)&&<span style={{fontSize:12,color:C.t3,fontStyle:"italic"}}>Sin subcategorías</span>}
        </div>
      </Card>
    ))}
    {catM&&<Modal title={catM==="new"?"Nueva categoría":"Editar categoría"} onClose={()=>setCatM(null)}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div><div style={{fontSize:11,color:C.t2,marginBottom:8,fontWeight:600}}>Ícono</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{ICONS.map(ic=><button key={ic} onClick={()=>setCatD(d=>({...d,icon:ic}))} style={{width:36,height:36,borderRadius:8,fontSize:18,border:`2px solid ${catD.icon===ic?C.green:"transparent"}`,background:catD.icon===ic?C.bg3:"transparent",cursor:"pointer"}}>{ic}</button>)}</div>
        </div>
        <div><div style={{fontSize:11,color:C.t2,marginBottom:8,fontWeight:600}}>Color</div>
          <div style={{display:"flex",gap:8}}>{COLS.map(col=><button key={col} onClick={()=>setCatD(d=>({...d,color:col}))} style={{width:26,height:26,borderRadius:"50%",background:col,border:`3px solid ${catD.color===col?"#fff":"transparent"}`,outline:catD.color===col?`2px solid ${col}`:"none",cursor:"pointer"}}/>)}</div>
        </div>
...
// ── PRESUPUESTO ───────────────────────────────────────────────────────────────
function PresupuestoTab({obra,gastos,presup,tcRef,cats,toast,reload}) {
  const [modal,setModal]=useState(false);
  const [draft,setDraft]=useState({cat_id:cats[0]?.id||"",monto:"",moneda:"ARS"});
  const [saving,setSaving]=useState(false);
  const totalPA=presup.reduce((s,p)=>s+(p.moneda==="USD"?p.monto*tcRef:p.monto),0);
  const totalEA=gastos.reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);

  const save=async()=>{
    if(!draft.monto||parseFloat(draft.monto)<=0)return;setSaving(true);
    const ex=presup.find(p=>p.cat_id===draft.cat_id);
    if(ex){
      const{error}=await supabase.from("presupuestos").update({monto:parseFloat(draft.monto),moneda:draft.moneda}).eq("id",ex.id);
      if(error){toast.error("Error");setSaving(false);return;}
    }else{
      const{error}=await supabase.from("presupuestos").insert({obra_id:obra.id,cat_id:draft.cat_id,monto:parseFloat(draft.monto),moneda:draft.moneda});
      if(error){toast.error("Error");setSaving(false);return;}
    }
  ...
  const doExport=()=>exportCSV([...presup.map(p=>{
    const cat=cats.find(c=>c.id===p.cat_id);
    const ej=gastos.filter(g=>g.cat_id===p.cat_id).reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);
    const pA=p.moneda==="USD"?p.monto*tcRef:p.monto;
    return{Categoria:cat?.label||"",Presupuesto:p.monto,Moneda:p.moneda,Presupuesto_ARS:Math.round(pA),Ejecutado_ARS:Math.round(ej),Diferencia:Math.round(pA-ej),Porc:pA>0?Math.round((ej/pA)*100)+"%":"—"};
  }),{Categoria:"TOTAL",Presupuesto:"—",Moneda:"—",Presupuesto_ARS:Math.round(totalPA),Ejecutado_ARS:Math.round(totalEA),Diferencia:Math.round(totalPA-totalEA),Porc:totalPA>0?Math.round((totalEA/totalPA)*100)+"%":"—"}],
  `presupuesto_${obra.nombre.replace(/\s+/g,"_")}.csv`);

  return <div className="fu">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
      <div><div style={{fontSize:16,fontWeight:700,color:C.t}}>Presupuesto</div><div style={{fontSize:12,color:C.t3}}>Presup: {fmtARS(totalPA)} · Ejec: {fmtARS(totalEA)}</div></div>
    ...
    {obra.presupuesto_total>0&&<div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14}}>
      <StatCard label="Presupuesto general" value={fmtM(obra.presupuesto_total,obra.moneda_presupuesto)} color={C.blue} icon="📐"/>
      <StatCard label="Ejecutado" value={fmtARS(totalEA)} color={C.green} icon="💸"/>
      <StatCard label="Disponible" value={fmtARS(Math.max(0,(obra.moneda_presupuesto==="USD"?obra.presupuesto_total*tcRef:obra.presupuesto_total)-totalEA))} color={C.lima} icon="✅"/>
    </div>}
    <Card>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:480}}>
          <thead><tr style={{borderBottom:`2px solid ${C.bd2}`}}>
            {["Categoría","Presupuesto","Ejecutado","Diferencia","Avance",""].map((h,i)=>(
          ...
          <tbody>
            {cats.map(cat=>{
              ...
              const ejA=gastos.filter(g=>g.cat_id===cat.id).reduce((s,g)=>s+(g.moneda==="USD"?g.monto*(g.tc_valor||tcRef):g.monto),0);
              if(!p&&ejA===0)return null;
              const pA=p?(p.moneda==="USD"?p.monto*tcRef:p.monto):0;
              const pct=pA>0?Math.min(Math.round((ejA/pA)*100),200):null;
              const diff=pA-ejA;
              ...
              return <tr key={cat.id} style={{borderBottom:`1px solid ${C.bd}`}}>
                ...
                <td style={{padding:"10px",textAlign:"right",color:C.t2,whiteSpace:"nowrap"}}>{p?fmtM(p.monto,p.moneda):"—"}</td>
                <td style={{padding:"10px",textAlign:"right",fontWeight:700,color:cat.color,whiteSpace:"nowrap"}}>{fmtARS(ejA)}</td>
                <td style={{padding:"10px",textAlign:"right",color:pA>0?(diff>=0?C.green:C.red):C.t3,fontWeight:600,whiteSpace:"nowrap"}}>{pA>0?fmtARS(diff):"—"}</td>
          ...
          <tfoot><tr style={{borderTop:`2px solid ${C.bd2}`,background:C.bg3}}>
            ...
            <td style={{padding:"10px",textAlign:"right",fontWeight:700,color:C.t}}>{fmtARS(totalPA)}</td>
            <td style={{padding:"10px",textAlign:"right",fontWeight:700,color:C.green}}>{fmtARS(totalEA)}</td>
            <td style={{padding:"10px",textAlign:"right",fontWeight:700,color:totalPA-totalEA>=0?C.green:C.red}}>{fmtARS(totalPA-totalEA)}</td>
            <td colSpan={2}/>
          </tr></tfoot>
        </table>
      </div>
...
function ParticipantesTab({obra,partic,toast,reload}) {
  ...
  const save=async()=>{
    if(!draft.email.trim()||!draft.nombre.trim())return;setSaving(true);
    const{data:profile}=await supabase.from("profiles").select("id").eq("email",draft.email).single();
    const{error}=await supabase.from("participantes").insert({
      obra_id:obra.id,user_id:profile?.id||null,
      email:draft.email.trim(),nombre:draft.nombre.trim(),
      rol:draft.rol,puede_cargar:draft.rol!=="cliente"?true:draft.puede_cargar,
    });
    if(error){toast.error("Error: "+error.message);setSaving(false);return;}
    toast.success(`${draft.nombre} invitado`);
  ...
  return <div className="fu">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      ...
      <Btn primary onClick={()=>setModal(true)}>+ Invitar</Btn>
    </div>
    <Card>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:400}}>
          <thead><tr style={{borderBottom:`2px solid ${C.bd2}`}}>
            {["Nombre","Email","Rol","Puede cargar",""].map((h,i)=><th key={i} style={{padding:"8px 10px",textAlign:"left",fontSize:10,fontWeight:700,color:C.t3,textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}
          </tr></thead>
          <tbody>
            {partic.map(p=><tr key={p.id} style={{borderBottom:`1px solid ${C.bd}`}}>
              ...
              <td style={{padding:"10px"}}>
                <select style={{...SEL,width:"auto",padding:"4px 8px",fontSize:11}} value={p.rol} onChange={e=>updateRol(p.id,e.target.value)}>
                  <option value="arquitecto">Arquitecto</option><option value="ayudante">Ayudante</option><option value="cliente">Cliente</option>
                </select>
              </td>
              <td style={{padding:"10px"}}>
                {p.rol==="cliente"?<label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}>
                  ...
                  <span style={{color:p.puede_cargar?C.green:C.t3,fontWeight:p.puede_cargar?600:400}}>{p.puede_cargar?"Sí":"Solo lectura"}</span>
                </label>:<span style={{color:C.green,fontWeight:600}}>✓ Sí</span>}
              </td>
              <td style={{padding:"10px",textAlign:"right"}}>{p.rol!=="arquitecto"&&<button onClick={()=>deleteP(p.id)} style={{background:"none",border:"none",cursor:"pointer",color:C.t3,fontSize:16}}>×</button>}</td>
            </tr>)}
          </tbody>
        </table>
      </div>
    </Card>
    <div style={{marginTop:12,background:C.bg3,border:`1px solid ${C.bd}`,borderRadius:10,padding:"12px 16px",fontSize:12,color:C.t3,display:"flex",gap:12,flexWrap:"wrap",alignItems:"center"}}>
      <Tag label="Arquitecto" color={C.green}/><span>acceso total</span>
      <Tag label="Ayudante" color={C.blue}/><span>igual, sin eliminar obra</span>
      <Tag label="Cliente" color={C.lima}/><span>solo ve resumen público</span>
    </div>
    {modal&&<Modal title="Invitar participante" onClose={()=>setModal(false)}>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        {draft.rol==="cliente"&&<label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",background:C.bg3,borderRadius:8,padding:"10px 12px"}}>
          ...
          <div><div style={{fontSize:12,fontWeight:600,color:C.t}}>Permitir cargar gastos</div><div style={{fontSize:11,color:C.t3}}>Si no, solo verá el resumen público</div></div>
        </label>}
  ...
  return <div className="fu">
    ...
    {inflData&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
        {acum.slice(-5).map(a=>(
          <div key={a.anio} style={{flex:"1 1 90px",background:C.bg2,border:`1px solid ${a.pct>100?C.red+"44":a.pct>50?C.amber+"44":C.green+"33"}`,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
            <div style={{fontSize:11,color:C.t3,marginBottom:4,fontWeight:600}}>{a.anio}</div>
            <div style={{fontSize:20,fontWeight:700,color:a.pct>100?C.red:a.pct>50?C.amber:C.green}}>{a.pct}%</div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",maxHeight:160,display:"block"}}>
          ...
          {u24.map((d,i)=>{
            const bW=Math.max(7,(W-PL-PR)/u24.length-3);
            const x=PL+(i/u24.length)*(W-PL-PR)+(W-PL-PR)/u24.length/2-bW/2;
            const bH=Math.max(2,(d.valor/maxI)*cH),y=PT+cH-bH;
            const col=d.valor>10?C.red:d.valor>5?C.amber:C.green;
            const show=i===0||i===u24.length-1||i%4===0;
            return <g key={i}><rect x={x} y={y} width={bW} height={bH} fill={col} rx="2" opacity=".8"/>{show&&<text x={x+bW/2} y={H-4} textAnchor="middle" fill={C.t3} fontSize="7" fontFamily="sans-serif">{d.fecha.slice(2,7)}</text>}</g>;
          })}
        </svg>
      </Card>
      <Card>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:380}}>
            <thead><tr style={{borderBottom:`2px solid ${C.bd2}`}}>{["Mes","IPC %","Equivalencia $1.000.000"].map((h,i)=><th key={i} style={{padding:"7px 10px",textAlign:i>=1?"right":"left",fontSize:10,fontWeight:700,color:C.t3,textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}</tr></thead>
            <tbody>{[...u24].reverse().slice(0,16).map(d=>{
              const col=d.valor>10?C.red:d.valor>5?C.amber:C.green;
              return <tr key={d.fecha} style={{borderBottom:`1px solid ${C.bd}`}}>
                <td style={{padding:"8px 10px",color:C.t2,fontWeight:500}}>{d.fecha.slice(0,7)}</td>
                <td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:col}}>{d.valor}%</td>
                <td style={{padding:"8px 10px",textAlign:"right",color:C.t3,fontSize:11}}>{fmtARS(1000000*(1+d.valor/100))}</td>
              </tr>;
            })}</tbody>
          </table>
        </div>
  ...
  return <div className="fu">
    ...
    {tcHistData&&<>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
        ...
        <StatCard label="Brecha" value={last?.gap!=null?`${last.gap}%`:"—"} color={last?.gap>100?C.red:last?.gap>50?C.amber:C.green} icon="📊"/>
      ...
      {sF.length>1&&<Card style={{marginBottom:14}}>
        ...
        {(()=>{
          const maxTC=Math.max(...br.map(x=>Math.max(x.of||0,x.bl||0)),1),n=br.length;
          ...
          return <><svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",maxHeight:170,display:"block"}}>
            {[0,.5,1].map((f,i)=>{const v=Math.round(maxTC*f),y=PT+cH*(1-f);return<g key={i}><line x1={PL} y1={y} x2={W-PR} y2={y} stroke={C.bd2} strokeWidth=".5"/><text x={PL-4} y={y+4} textAnchor="end" fill={C.t3} fontSize="8" fontFamily="sans-serif">{v>=1000?Math.round(v/1000)+"K":v}</text></g>;})}
            ...
            {br.map((b,i)=>{const x=PL+(i/(n-1||1))*(W-PL-PR);const s=i===0||i===n-1||i%4===0;return s?<text key={i} x={x} y={H-4} textAnchor="middle" fill={C.t3} fontSize="8" fontFamily="sans-serif">{b.label}</text>:null;})}
          </svg>
      ...
      <Card>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:380}}>
            <thead><tr style={{borderBottom:`2px solid ${C.bd2}`}}>{["Mes","Oficial","Blue","Brecha","IPC"].map((h,i)=><th key={i} style={{padding:"7px 10px",textAlign:i>=1?"right":"left",fontSize:10,fontWeight:700,color:C.t3,textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}</tr></thead>
            <tbody>{[...br].reverse().slice(0,18).map((b,i)=>(
              <tr key={i} style={{borderBottom:`1px solid ${C.bd}`}}>
                ...
                <td style={{padding:"8px 10px",textAlign:"right",color:b.gap>100?C.red:b.gap>50?C.amber:C.t2,fontWeight:b.gap?600:400}}>{b.gap!=null?`${b.gap}%`:"—"}</td>
